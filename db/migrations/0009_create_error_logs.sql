...
CREATE TABLE
  IF NOT EXISTS error_logs (
    ...
    -- User identification (nullable for errors without user context)
    user_id UUID,
...
COMMENT ON COLUMN error_logs.user_id IS 'User associated with the error (nullable)';
...
COMMENT ON COLUMN error_logs.context IS 'Additional error context as JSONB (request data, environment, etc.)';
