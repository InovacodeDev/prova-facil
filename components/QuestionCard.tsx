...
export const QuestionCard = ({ question }: QuestionCardProps) => {
    ...
    // Helper function to get display content based on question type
    const getQuestionContent = () => {
        const metadata = question.metadata || {};

        // Use metadata if available, otherwise fall back to answers table
        if (metadata && Object.keys(metadata).length > 0) {
            switch (question.type) {
                ...
                case "matching_columns":
                    return {
                        columnA: metadata.column_a || [],
                        columnB: metadata.column_b || [],
                        matches: metadata.correct_matches || [],
                    };
                ...
                case "problem_solving":
                    return metadata.evaluation_criteria || metadata.rubric || [];
                case "project_based":
                    return {
            ...
            case "sum":
                if (Array.isArray(content) && content.length > 0) {
                    return (
                        <div className="space-y-2">
                            <p className="text-xs text-muted-foreground mb-3">
                                Marque as afirmativas corretas e some seus valores:
                            </p>
                            {content.map((item: any, index: number) => (
                                <div
                                    key={index}
            ...
            case "problem_solving":
                return (
                    <div className="space-y-3">
                        ...
                        {isFlipped && Array.isArray(content) && content.length > 0 && (
                            <div className="mt-4 pt-4 border-t border-border">
                        ...
                        {isFlipped && question.answers.length > 0 && !Array.isArray(content) && (
                            <div className="mt-4 pt-4 border-t border-border">
                                <p className="text-xs font-semibold text-muted-foreground mb-2">Resposta esperada:</p>
                                <p className="text-sm">{question.answers[0]?.answer}</p>
                            </div>
                        )}
                    </div>
                );
                    ...
                    <div className="space-y-3">
                        {content.sections && content.sections.length > 0 ? (
                            ...
                            <div className="p-3 bg-muted rounded-lg text-sm">
                                Avaliação somativa (múltiplas seções e critérios)
                            </div>
                        )}
                    </div>
                );
