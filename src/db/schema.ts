...
import { pgTable, varchar, text, timestamp, boolean, jsonb, uuid, pgEnum, integer } from 'drizzle-orm/pg-core';
...
export const academicLevelSubjects = pgTable('academic_level_subjects', {
  id: uuid('id').defaultRandom().primaryKey().notNull(),
  academic_level_id: uuid('academic_level_id').references(() => academicLevels.id).notNull(),
  subject_id: uuid('subject_id').references(() => subjects.id).notNull(),
  created_at: timestamp('created_at').defaultNow().notNull(),
});
...
export const assessmentsRelations = relations(assessments, ({ one, many }) => ({
  ...
  category: one(categories, { fields: [assessments.category_id], references: [categories.id] }),
...
export const academicLevelSubjectsRelations = relations(academicLevelSubjects, ({ one }) => ({
  academicLevel: one(academicLevels, { fields: [academicLevelSubjects.academic_level_id], references: [academicLevels.id] }),
